...
%% method
flag_SMV = 0; %consider the single antenna BS (SMV-OMP)
...
reals = 1e6; %num. channel realization (e.g. 1e6)
...
%         for sn = 1:length(SNR)
    for sn = 1:length(SNR)
        ...
        for iter = 1:reals
            ...
            % noise generation
            n = sqrt(0.50 * nvar) * (randn(M, Nbs) + 1j * randn(M, Nbs));
    ...
    for sn = 1:length(SNR)
        ...
        for iter = 1:reals
            ...
            if (frame_GRF)
                ...
                % OMP
                [xhat_GRF, SetEst_GRF] = MMVOMP_nzknown(y, A_GRF, K);
            ...
            if (frame_DFT)
                ...
                % OMP
                [xhat_DFT, SetEst_DFT] = MMVOMP_nzknown(y, A_DFT, K);
            ...
            if (frame_QCS)
                ...
                % OMP
                [xhat_QCS, SetEst_QCS] = MMVOMP_nzknown(y, A_QCS, K);
